...
class Button(pygame.sprite.Sprite):
    def __init__(self, surf, disabled_surf, pos, size, groups):
        super().__init__(groups)
        self.text = ''
        self.image = pygame.transform.smoothscale(surf, size)
        self.image_disabled = pygame.transform.smoothscale(disabled_surf, size)
        self.rect = self.image.get_frect(center=pos)
        self.is_active = True
        self.text_sprite = TextSprite('', self.rect.center, "darkcyan", self.rect.width, self.rect.width / 8, ())
...
class InteractiveButton(Button):
    def __init__(self, surf, hover_surf, disabled_surf, pos, size, groups):
        super().__init__(surf, disabled_surf, pos, size, groups)
