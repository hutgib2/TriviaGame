...
class TriviaGame:
    def __init__(self):
        ...
        self.choice = None
        self.round_number = 0
        self.question_sprite = TextSprite(
            "",
            (5 * WINDOW_WIDTH / 8, WINDOW_HEIGHT / 8),
            "white",
            2 * WINDOW_WIDTH / 3,
            WINDOW_WIDTH / 32,
            (self.all_sprites),
        ...
        self.magic_cup_timer = Timer(
            2000, lambda: {cup.kill() for cup in self.magic_cups}
        )
    ...
    async def fetch_questions(self):
        ...
        if len(self.easy_questions) < 7:
            self.easy_questions.append(
                random.sample(self.easy_backup_qs, k=(7 - len(self.easy_questions)))
            )
        if len(medium_questions) < 5:
            medium_questions.append(
                random.sample(self.medium_backup_qs, k=(5 - len(self.medium_questions)))
            )
        if len(hard_questions) < 5:
            hard_questions.append(
                random.sample(self.hard_backup_qs, k=(5 - len(self.hard_questions)))
            )

        self.questions = (
            self.easy_questions[:5]
            + random.sample(medium_questions, k=5)
            + random.sample(hard_questions, k=5)
        )
    ...
    def activate_magic_cup(self):
        ...
        for i in range(len(self.activated_buttons)):
            Cup(
                MAGIC_CUPS["POS"][i], numbers.pop(), (self.magic_cups, self.all_sprites)
            )
    ...
    def create_prize_tree(self):
        ...
        for i in range(70, 1, -5):
            Button(
                PRIZE_BUTTONS,
                (WINDOW_WIDTH / 10, (i * WINDOW_HEIGHT / 75)),
                (WINDOW_WIDTH / 11, WINDOW_HEIGHT / 15),
                (self.prize_buttons, self.all_sprites),
            )
    ...
    def start_game(self):
        ...
        InteractiveButton(
            GAME_BUTTONS["SURFS"],
            GAME_BUTTONS["POS"]["top_left"],
            GAME_BUTTONS["size"],
            (self.game_buttons, self.all_sprites),
            self.check_result,
        )
        InteractiveButton(
            GAME_BUTTONS["SURFS"],
            GAME_BUTTONS["POS"]["top_right"],
            GAME_BUTTONS["size"],
            (self.game_buttons, self.all_sprites),
            self.check_result,
        )
        ...
        InteractiveButton(
            ...
            LIFELINES["POS"][0],
            LIFELINES["size"],
            (self.lifelines, self.all_sprites),
            self.activate_x2,
            "X2",
        )
        InteractiveButton(
        ...
        InteractiveButton(
            ...
            WALK_AWAY["size"],
            (self.lifelines, self.all_sprites),
            lambda: self.end_game("walk_away"),
            "",
        )
    ...
    def update_current_question(self, question):
        ...
        answers = [
            ...
            self.current_question["incorrect_answers"][0],
            self.current_question["incorrect_answers"][1],
            self.current_question["incorrect_answers"][2],
        ]
